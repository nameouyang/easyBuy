jQuery(function() {
    ...
    uploader2 = WebUploader.create({
        pick: {
            id: '#filePicker3',
            label: '<span class="textdemo"></span>'
        },
        ...
        formData:{
            ...
            'FileUploader[remark]':'screen config template',
        },
        fileVal:'FileUploader[file]',

        chunked: false,
        // server: 'http://webuploader.duapp.com/server/fileupload.php',
        server: '/util/file/upload',
        fileNumLimit: 300,
    ...
    function setState( val ) {
        ...
        switch ( state ) {
            ...
            case 'finish':
                stats = uploader2.getStats();
                if ( stats.successNum ) {
                    Utils.Toastr.Success('成功', '上传图片成功');
                    setTimeout(function(){
    ...
    uploader2.on( 'uploadSuccess', function( file,response ) {
        ...
        var path = '//i6.mifile.cn'+response.data;
        $( '#'+file.id ).addClass('upload-state-done');
        updatePic('add','poster',path);
    });
    ...
    function updatePic(action,name,path){
        var id=$('#template_id').val();
        if(action == 'add'){
            var url = '/product-template/add-pic?id='+id;
        }
        if(action == 'minus'){
            var url = '/product-template/remove-pic?id='+id;
        }
        $.ajax({
            type: "POST",
            url:url,
            data: {"name":name,"pic":path},
            dataType: "json",
            success: function(data){
                if(data.code != '0'){
                    Utils.Toastr.Error('失败', '添加图片成功');
                }
            }
        });
    }
